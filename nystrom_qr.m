function [cluster_labels, evd_time, total_time] = nystrom_qr(Z, permed_index, num_clusters)
...
[Q , R] = qr(Z, 0); % thin QR decomposition of matrix Z对矩阵进行QR分解
% Calculate M = R * A^(-1) * R'
M = (R * pinv(A) * R');
...
% Calculate orthogonal eigenvector V = Q * U
V = Q * U(:, 1:num_clusters);
clear Q U;
time2 = toc;
